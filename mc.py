...
def mc_control(n_episodes: int = 1000) -> Dict[State, Dict[Action, float]]:
    ...
    """
    state_to_action_count = defaultdict(lambda: {Action.HIT: 0, Action.STICK: 0})
    state_to_action_value = defaultdict(lambda: {Action.HIT: 0, Action.STICK: 0})
    ...
    for _ in range(n_episodes):
        ...
        while not s.is_terminal:
            ...
            action_values = state_to_action_value[s]
            ...
            N_s = sum(ct for ct in state_to_action_count[s].values())
        ...
        for s, a, r in history:
            if not seen[s][a] and not s.is_terminal:  # first-visit evaluation
                state_to_action_count[s][a] += 1  # increment visit count
                step_size = state_to_action_count[s][a]
                q = state_to_action_value[s][a]
                state_to_action_value[s][a] += (final_reward - q) / step_size
                seen[s][a] = True
    return state_to_action_value
